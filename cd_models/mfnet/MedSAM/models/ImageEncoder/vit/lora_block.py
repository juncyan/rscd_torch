...
class LoraBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        ...
        x = shortcutx + x
        y = shortcuty + y
        mlpx, mlpy = self.mlp(self.norm2(x), self.norm2(y))
        x = mlpx + x
        y = mlpy + y
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        ...
        x = x.reshape(B,H*W,n)
        y = y.reshape(B,H*W,n)
        qkvx, qkvy = self.qkv(x, y)
        qkvx = qkvx.reshape(B, H * W, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        qkvy = qkvy.reshape(B, H * W, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        # q, k, v with shape (B * nHead, H * W, C)
        qx, kx, vx = qkvx.reshape(3, B * self.num_heads, H * W, -1).unbind(0)
        qy, ky, vy = qkvy.reshape(3, B * self.num_heads, H * W, -1).unbind(0)

        attnx = (qx * self.scale) @ kx.transpose(-2, -1)
        attny = (qy * self.scale) @ ky.transpose(-2, -1)

        if self.use_rel_pos:
            attnx = add_decomposed_rel_pos(attnx, qx, self.rel_h, self.rel_w, (H, W), (H, W))
            attny = add_decomposed_rel_pos(attny, qy, self.rel_h, self.rel_w, (H, W), (H, W))

        attnx = attnx.softmax(dim=-1)
        x = (attnx @ vx).view(B, self.num_heads, H, W, -1).permute(0, 2, 3, 1, 4).reshape(B, H, W, -1)
        x = self.proj(x)

        attny = attny.softmax(dim=-1)
        y = (attny @ vy).view(B, self.num_heads, H, W, -1).permute(0, 2, 3, 1, 4).reshape(B, H, W, -1)
        y = self.proj(y)
