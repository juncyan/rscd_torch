...
from .unet import UNet, UNet_DS, UNet_URPC, UNet_CCT
import argparse
...
def net_factory(net_type="unet", in_chns=3, class_num=5, img_size=512):
    if net_type == "unet":
        net = UNet(in_chns=in_chns, class_num=class_num)
    elif net_type == "enet":
        net = ENet(in_channels=in_chns, num_classes=class_num)
        ...
        net = ViT_seg(in_chns=in_chns, img_size=img_size, num_classes=class_num)
    elif net_type == "pnet":
        net = PNet2D(in_chns, class_num, 64, [1, 2, 4, 8, 16])
    elif net_type == "nnUNet":
        net = initialize_network(num_classes=class_num)
    elif net_type == "mambau":
        net = MambaUnet(in_chns=in_chns, img_size=img_size,num_classes=class_num)
    else:
        net = None
    return net
