...
class ISDANet(nn.Module):
    ...
    def loss(preds, label):
        pred, aux1, aux2, aux3 = preds
        loss_pred = criterion(pred, label)
        loss_aux_1 = criterion(aux1, label)
        loss_aux_2 = criterion(aux2, label)
        loss_aux_3 = criterion(aux3, label)
    ...
    def predict(preds):
        pred, _, _, _ = preds
        return F.softmax(pred, dim=1).max(dim=1)[1]
