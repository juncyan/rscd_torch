...
criterion_sem = LatentSimilarity(T=3.0).to(torch.device('cuda:0'))
...
#     @ARTICLE{10443350,
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor):
        ...
        featA_s4 = self.Adapter4(featsA[3].clone().to(self.device))
        featA_s8 = self.Adapter8(featsA[0].clone().to(self.device))
        featA_s16 = self.Adapter16(featsA[1].clone().to(self.device))
        featA_s32 = self.Adapter32(featsA[2].clone().to(self.device))
        ...
        featB_s4 = self.Adapter4(featsB[3].clone().to(self.device))
        featB_s8 = self.Adapter8(featsB[0].clone().to(self.device))
        featB_s16 = self.Adapter16(featsB[1].clone().to(self.device))
        featB_s32 = self.Adapter32(featsB[2].clone().to(self.device))              
    ...
    def loss(pred, label):
        ...
        loss_bn = F.binary_cross_entropy_with_logits(p, lab)
