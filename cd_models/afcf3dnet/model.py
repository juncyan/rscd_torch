...
from .decom_backbone import ResNet3D
# from model.ResNet18 import ResNet18_pure
...
#   doi={10.1109/TGRS.2023.3305499}}
    ...
    def __init__(self):
        ...
        self.resnet = ResNet3D(copy.deepcopy(resnet))
    ...
    def loss(target, gt):
        ...
        bce_loss = bce(target, gt)

        # true_u = gt.unsqueeze(1)
        # target_u = target.unsqueeze(1)

        inter = (gt * target).sum()
        eps = 1e-7
        dice_loss = (2 * inter + eps) / (gt.sum() + target.sum() + eps)

        return bce_loss + 1 - dice_loss
    
    @staticmethod
    def predict(preds):
        bin_genmask = (preds > 0.5).long()
        bin_genmask = bin_genmask.detach().cpu().numpy()
        return bin_genmask



# def BCEDICE_loss(target, gt):
#     bce = torch.nn.BCELoss()
#     bce_loss = bce(target, gt)
