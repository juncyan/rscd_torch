...
class Decoder(nn.Module):
    ...
    def forward(self, d2, d3, d4, d5):
        ...
        p4 = self.conv_p4(d4 + F.interpolate(p5, size=d4.shape[-2:], mode='bilinear'))

        p4, mask_map_p4 = self.SR3(p4)
        p3 = self.conv_p3(d3 + F.interpolate(p4, size=d3.shape[-2:], mode='bilinear'))

        p3, mask_map_p3 = self.SR4(p3)
        p2 = self.conv_p2(d2 + F.interpolate(p3, size=d2.shape[-2:], mode='bilinear'))
...
class EAFHNet(nn.Module):
    def __init__(self, input_nc=3, output_nc=1):
        super(EAFHNet, self).__init__()
    ...
    def forward(self, x1, x2=None):
        ...
        x1_4 = F.interpolate(x1_4, scale_factor=2, mode='bilinear')
        x1_5 = F.interpolate(x1_5, scale_factor=4, mode='bilinear')
        x2_4 = F.interpolate(x2_4, scale_factor=2, mode='bilinear')
        x2_5 = F.interpolate(x2_5, scale_factor=4, mode='bilinear')
        # temporal fusion
        ...
        mask_map_p4 = F.interpolate(mask_map_p4, scale_factor=(8, 8), mode='bilinear')
        mask_map_p4 = torch.sigmoid(mask_map_p4)
        mask_map_p5 = F.interpolate(mask_map_p5, scale_factor=(8, 8), mode='bilinear')
    ...
    def loss(preds, target):
        ...
        out, out2, out3, out4 = preds
        loss = BCEDiceLoss(out, target) + BCEDiceLoss(out2, target) + BCEDiceLoss(out3, target) + BCEDiceLoss(out4, target)
        return loss
