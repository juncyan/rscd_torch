...
class BiSRNet(nn.Module):
    ...
    def __init__(self, in_channels=3, num_classes=2):
        ...
        self.CD = nn.Sequential(nn.Conv2d(128, 64, kernel_size=1), nn.BatchNorm2d(64), nn.ReLU(), nn.Conv2d(64, num_classes, kernel_size=1))
        initialize_weights(self.head, self.SiamSR, self.res1, self.CD)
    ...
    def base_forward(self, x):
        # f, _ =  self.PSPNet.feats(x)
        # print(f.shape)
        p = self.Unetplusplus(x) #8x3x256x256
        p = self.head(p)   # 8, 512, 32, 32  / 8, 128, 32, 32
        return p
    # def base_forward(self, x):
    #
