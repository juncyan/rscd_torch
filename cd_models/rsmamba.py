...
if __name__ == "__main__":
    ...
    device = "cuda:0"
    cd_model = RSMamba_CD(drop_path_rate=0.2, dims=96, depths=[ 2, 2, 9, 2 ], ssm_d_state=16, ssm_dt_rank="auto",
                      ssm_ratio=2.0, mlp_ratio=4.0, downsample_version="v3", patchembed_version="v2",
                      image_size=256, downsample_raito=1).to(device)
    img = torch.randn(1, 3, 256, 256).to(device)
    res = cd_model(img, img)
    print(res.shape)
    # print(cd_model.flops(shape=(3, 256, 256)))
    # param_num = 0
