...
def build_sam_vit_t(img_size=512):
    ...
    sam_checkpoint = "/mnt/data/weights/mobile_sam.pt"
    ...
    mobile_sam.load_state_dict(OrderedDict(model_dict), strict=True) # 加载预训练参数
    # mobile_sam.eval()#测试打开
...
# #     # net.load_state_dict(torch.load( sam_checkpoint ))
...
# #         else:
# #             print(k)
# #         # 处理新模型中没有的权重，如果有多余的权重，可以选择忽略或手动处理
# #     # model_dict.update(pretrained_dict)# 将预训练参数更新到新的网络层

# #     # net.load_state_dict(OrderedDict(model_dict), strict=True) # 加载预训练参数
