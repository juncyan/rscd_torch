...
class AFPN(nn.Module):
    ...
    def forward(self, ms_feats):
        ...
        x0 = self.stage1_Conv1(torch.cat([fa1, fb1], 1))  # inplanes
        x1 = self.stage2_Conv1(torch.cat([fa2, fb2], 1))  # inplanes * 2
        x2 = self.stage3_Conv1(torch.cat([fa3, fb3], 1))  # inplanes * 2
        x3 = self.stage4_Conv1(torch.cat([fa4, fb4], 1))  # inplanes * 8
        ...
        # out1 = F.interpolate(out1, size=(change1_h, change1_w), mode='bilinear', align_corners=True)
        # out2 = F.interpolate(out2, size=(change1_h, change1_w), mode='bilinear', align_corners=True)
        # out3 = F.interpolate(out3, size=(change1_h, change1_w), mode='bilinear', align_corners=True)
...
if __name__ == '__main__':
    network = AFPN(inplanes=256)
    input1 = torch.rand(2, 256, 64, 64)
    input2 = torch.rand(2, 512, 32, 32)
    input3 = torch.rand(2, 1024, 16, 16)
    input4 = torch.rand(2, 2048, 8, 8)

    input = input1,input2,input3,input4,input1,input2,input3,input4
