...
class ResidualMambaEncoder(nn.Module):
    ...
    def forward(self, x):
        print("ResidualMambaEncoder input shape: ", x.shape)
        if self.stem is not None:
            x = self.stem(x)
        ret = []
        print("ResidualMambaEncoder step 1 shape: ", x.shape)
        #for s in self.stages:
        for s in range(len(self.stages)):
            #x = s(x)
            x = self.stages[s](x)
            print("ResidualMambaEncoder after resblock shape: ", x.shape)
            x = self.mamba_layers[s](x)
            print("ResidualMambaEncoder after mamba layer shape: ", x.shape)
            ret.append(x)
...
def EM_UNet(input_channels=1, num_classes=9):
    m = UMambaEnc(input_channels=input_channels,
...
if __name__ == "__main__":
    model = UMambaEnc(input_channels=1,
                      deep_supervision=True,
                      num_classes=9,
                      n_stages=6,
                      conv_op = nn.modules.conv.Conv3d,
    ...
    # macs, params = get_model_complexity_info(copy.deepcopy(model).cuda(), (1, 128, 128, 128), as_strings=True, print_per_layer_stat=False)
    # print("%s |%s |%s" % ("UMamba Enc", macs, params))
