...
from datasets.MusReader import MusReader
...
def main():        
    # net = Net(3, num_classes=args['num_classes']).cuda(args['device'])
    os.environ['CUDA_VISIBLE_DEVICES'] = "{}".format(args['device'])
    net = SSCDl(3, num_classes=args['num_classes']).cuda(args['device'])
    net = net.to(args['device'], dtype=torch.float)
    # net = nn.DataParallel(net)
    #net.load_state_dict(torch.load(args['load_path']), strict=False)
        
    train_set = MusReader(r"/mnt/data/Datasets/MusSCD",'train')
    train_loader = DataLoader(train_set, batch_size=args['train_batch_size'], num_workers=4, shuffle=True)
    val_set = MusReader(r"/mnt/data/Datasets/MusSCD",'val')
    val_loader = DataLoader(val_set, batch_size=args['val_batch_size'], num_workers=4, shuffle=False)
    
    criterion = CrossEntropyLoss2d(ignore_index=0).cuda(args['device'])
...
def train(train_loader, net, criterion, optimizer, scheduler, val_loader):
    ...
    criterion_sc = ChangeSimilarity().cuda(args['device'])
    curr_epoch=0
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            if args['gpu']:
                imgs_A = imgs_A.cuda(args['device']).float()
                imgs_B = imgs_B.cuda(args['device']).float()
                labels_bn = (labels_A>0).unsqueeze(1).cuda(args['device']).float()
                labels_A = labels_A.cuda(args['device']).long()
                labels_B = labels_B.cuda(args['device']).long()

            optimizer.zero_grad()
            out_change, outputs_A, outputs_B = net(imgs_A, imgs_B)
            
            assert outputs_A.size()[1] == args['num_classes']
            ...
            # print(labels_A.shape,labels_B.shape,outputs_A.shape,outputs_B.shape,loss_seg,loss_bn)
...
def validate(val_loader, net, criterion, curr_epoch):
    ...
    Fscd, IoU_mean, Sek = SCDD_eval_all(preds_all, labels_all, args['num_classes'])
