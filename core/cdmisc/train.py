...
def train(model, dataloader_train, dataloader_eval, dataloader_test, args):
    ...
    for epoch in range(args.iters):
        ...
        for image1, image2, label in tqdm(dataloader_train):
        ...
        if miou_eval > max_iou:
            torch.save(model.state_dict(), args.best_model_path)
            max_iou = miou_eval
            best_iter = epoch+1
        args.logger.info("[TRAIN] train time is {:.2f}, best iter {}, max IoU {:.4f}".format((datetime.datetime.now() - now).total_seconds(), best_iter, max_iou))
