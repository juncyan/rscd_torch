...
class LKSSMBlock_v2(nn.Module):
    def __init__(self,
        ...
        self.embeding = nn.Conv2d(dim, 3*dim, 1)

        self.se = ChannelOperation(dim)#SEBlock(dim, dim // 4)
        self.ssm = SS2D_v3(dim, dim)
        # self.conv = nn.Sequential(nn.Conv2d(dim, dim, 7, 1, 3, groups=dim), nn.BatchNorm2d(dim))
    ...
    def compute_residual(self, x):
        ...
        # y = y + self.ssm(x)
        #mode 3
        q, k, v = self.embeding(x).chunk(3, dim=1)
        q = q.contiguous()
        y1 = self.se(self.norm(self.dwconv(q)))
        y2 = self.ssm(k)
        y = (y1 + y2) * v 
        # y = y * 
        if self.gamma is not None:
