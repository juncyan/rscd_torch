...
from .replk import SS2D_v3, DilatedReparamBlock, LKSSMBlock, SS2D_v4, LKSSMBlock_v2
...
class RepLKSSMBlock_v2(nn.Module):
    def __init__(self, in_channels, kernel_sizes=[7, 13] , activation='relu6'):
        super().__init__()
        self.dwconvs = nn.ModuleList([
            # nn.Sequential(RepLKSSMLayer(in_channels, kernel_size, activation))
            LKSSMBlock_v2(in_channels, kernel_size)
            for kernel_size in kernel_sizes])
...
class CrossDimensionalGroupedAggregation_v2(nn.Module):
    def __init__(self,F_g, F_l, F_int, activation='relu'):
        super().__init__()
        ...
        self.rlk = LKSSMBlock_v2(F_int, 13)#nn.Sequential(RepLKSSMLayer(F_int, 13, activation))

        # self.ssm = SS2D_v3(F_int, F_int)
        self.att = nn.Sequential(nn.Conv2d(F_int, F_int, 3,1,1), nn.BatchNorm2d(F_int), nn.SiLU())#AdditiveTokenMixer_L(F_int)
