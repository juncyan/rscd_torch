...
from .replk import SS2D_v3, DilatedReparamBlock, LKSSMBlock, LKSSMBlock_v2, SS2D_v4
...
class DTMS_v3(nn.Module):
    #Dual token modeling SSM
    def __init__(self, in_ch, out_ch, kernels_size=13, channel_first=True):
        super().__init__()
        self.channel_first = channel_first
        dim1 = 2*in_ch
        self.ssm1 = SS2D_v4(dim1, out_ch)
        self.ssm2 = SS2D_v4(in_ch, out_ch)
        
        self.conv = ConvBNAct(2*in_ch, out_ch, 1)
        self.cbr1 = ConvBNAct(3*out_ch, out_ch, 1)
        self.cbr2 = LKSSMBlock_v2(out_ch, kernels_size)#AdditiveTokenMixer_v2(out_ch, kernels_size)
        self.cbr3 = ConvBNAct(out_ch, out_ch, 3, padding=1)
